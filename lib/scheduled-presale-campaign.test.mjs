...
test("Hobby UI distinguishes presale opening from the next daily automatic run", async () => {
  ...
  assert.match(source, /Automatic email:/);
  assert.match(source, /nextDailySchedulerRun/);
  assert.match(source, /setUTCHours\(4, 15, 0, 0\)/);
  assert.match(source, /Vercel Hobby runs this scheduler once daily/);
  assert.match(source, /remains Scheduled until the next daily run/);
});
