...
const ambiguityFixMigration = readFileSync(
  "supabase/migrations/20260910_fix_sponsor_comp_redemption_token_undo_ambiguity.sql",
  "utf8",
).replace(/\r\n/g, "\n");
...
test("deployed undo replacement qualifies output-column names in updates", () => {
  assert.match(ambiguityFixMigration, /create or replace function public\.undo_sponsor_comp_redemption_token/);
  assert.match(ambiguityFixMigration, /update public\.show_sponsor_comp_redemption_tokens as token_row/);
  assert.match(ambiguityFixMigration, /and token_row\.show_sponsor_id = v_sponsor\.id/);
  assert.doesNotMatch(
    ambiguityFixMigration,
    /\band show_sponsor_id = v_sponsor\.id/,
  );
});
...
test("undo API and Door Mode preserve success shape and expose safe RPC diagnostics", () => {
  assert.match(route, /console\.info\("Sponsor comp barcode undo request\.", \{ showId, tokenId \}\)/);
  assert.match(route, /rpcError: rpcErrorDiagnostic\(error\)/);
  assert.match(route, /resultStatus: row\?\.result_status \?\? null/);
  assert.match(route, /\{ success: true, result \}/);
  assert.match(route, /Sponsor ticket undo RPC\$\{code\}: \$\{diagnostic\.message\}/);
  assert.match(doorPage, /httpStatus: response\.status/);
  assert.match(doorPage, /responseBody: payload/);
  assert.match(doorPage, /resultStatus: payload\.result\.resultStatus/);
  assert.match(doorPage, /Unable to undo sponsor ticket: \$\{detail\}/);
  assert.doesNotMatch(doorPage, /throw new Error\("Unable to undo this sponsor ticket check-in\."\)/);
});
...
test("successful sponsor scan result exposes the exact-token undo as its primary action", () => {
  const sponsorResult = sourceBetween(
    doorPage,
    '              {scanState.kind === "sponsor_comp_redemption" ? (',
    '              {scanState.kind === "sponsor_comp_redemption_undone" ? (',
  );
  assert.match(sponsorResult, /resultStatus === "REDEEMED"/);
  assert.match(sponsorResult, /handleUndoScannedSponsorComp/);
  assert.match(sponsorResult, /Undo This Check-In/);
  assert.match(doorPage, /item\.sponsorCompTokenId === scanState\.lookup\.redemption\.tokenId/);
  assert.match(doorPage, /sponsorCompTokenId: redemption\.tokenId/);
});

test("non-success sponsor results and normal admissions never show sponsor token undo", () => {
  const sponsorResult = sourceBetween(
    doorPage,
    '              {scanState.kind === "sponsor_comp_redemption" ? (',
    '              {scanState.kind === "sponsor_comp_redemption_undone" ? (',
  );
  const undoConditionIndex = sponsorResult.lastIndexOf('resultStatus === "REDEEMED" ? (');
  const undoButtonIndex = sponsorResult.indexOf("Undo This Check-In", undoConditionIndex);
  assert.ok(undoConditionIndex >= 0 && undoButtonIndex > undoConditionIndex);
  assert.equal((sponsorResult.match(/Undo This Check-In/g) ?? []).length, 1);

  const normalResult = sourceBetween(
    doorPage,
    '              {scanState.kind === "found" ? (',
    '            </div>\n          ) : null}',
  );
  assert.doesNotMatch(normalResult, /Undo This Check-In|handleUndoScannedSponsorComp/);
});

test("successful dedicated undo keeps authoritative totals visible and removes its button", () => {
  const undoneResult = sourceBetween(
    doorPage,
    '              {scanState.kind === "sponsor_comp_redemption_undone" ? (',
    '              {scanState.kind === "found" ? (',
  );
  assert.match(undoneResult, /Checked In: \{scanState\.result\.checkedIn\}/);
  assert.match(undoneResult, /Remaining: \{scanState\.result\.remaining\}/);
  assert.match(undoneResult, /Check-In Undone/);
  assert.doesNotMatch(undoneResult, /Undo This Check-In/);
});
...
test("dedicated undo invalidates stale scan responses and blocks overlapping scans", () => {
  assert.match(doorPage, /const scanRequestGenerationRef = useRef\(0\)/);
  assert.match(doorPage, /scanRequestGenerationRef\.current \+= 1/);
  assert.match(doorPage, /const scanRequestGeneration = \+\+scanRequestGenerationRef\.current/);
  assert.match(doorPage, /scanRequestGeneration !== scanRequestGenerationRef\.current/);
  assert.match(doorPage, /if \(!show \|\| activeActionId\)/);
  assert.match(doorPage, /disabled=\{isScanLookupPending \|\| Boolean\(activeActionId\)\}/);
});
