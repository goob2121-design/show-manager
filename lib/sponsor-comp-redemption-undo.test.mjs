...
test("successful sponsor scan result exposes the exact-token undo as its primary action", () => {
  ...
  assert.match(doorPage, /item\.id === scanState\.activityId/);
...
test("repeated redemption cycles bind scan-result undo to a fresh activity instance", () => {
  assert.match(doorPage, /sponsorCompActivityId = `sponsor-comp-token-\$\{redemption\.tokenId\}-\$\{crypto\.randomUUID\(\)\}`/);
  assert.match(doorPage, /id: sponsorCompActivityId/);
  assert.match(doorPage, /activityId: sponsorCompActivityId/);

  const firstCycle = { id: "token-a-cycle-1", tokenId: "token-a", undo: null };
  const secondCycle = { id: "token-a-cycle-2", tokenId: "token-a", undo: async () => {} };
  const tokenB = { id: "token-b-cycle-1", tokenId: "token-b", undo: async () => {} };
  const activities = [secondCycle, firstCycle, tokenB];
  const selected = activities.find((item) =>
    item.id === secondCycle.id && item.tokenId === "token-a" && item.undo,
  );

  assert.equal(selected, secondCycle);
  assert.notEqual(selected, firstCycle);
  assert.equal(tokenB.undo instanceof Function, true);
});
