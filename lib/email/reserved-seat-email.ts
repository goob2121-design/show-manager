...
function buildTicketCodeSection(input: ReservedSeatEmailInput) {
  if (!input.scanToken) {
    return { html: "", text: "" };
  }
  ...
  try {
    ...
    const ticketCodeHtml = display.code128DataUri
      ? `<div style="margin-top:14px;text-align:center;"><img src="${display.code128DataUri}" alt="Reservation barcode" width="300" style="display:block;width:100%;max-width:300px;height:auto;margin:0 auto;border:0;"><p style="margin:8px 0 0;color:#475569;font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;">Ticket Code: ${escapeHtml(display.shortReference)}</p></div>`
      : "";
    const qrHtml = display.qrDataUri
      ? `<div style="text-align:center;"><img src="${display.qrDataUri}" alt="Reservation QR code" width="200" height="200" style="display:block;width:100%;max-width:200px;height:auto;margin:0 auto;border:0;background:#ffffff;"></div>`
      : "";
