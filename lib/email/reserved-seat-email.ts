...
export function buildReservedSeatEmail(input: ReservedSeatEmailInput, logoSrc: string | null = CMMS_EMAIL_LOGO_SRC): ReservedSeatEmailContent {
  ...
  const seatSelectionCodeReminder = input.scanToken?.trim()
    ? `<p style="margin:0 0 24px;padding:12px 14px;background:#fff7ed;border-left:4px solid #d89b2b;color:#334155;font-size:14px;line-height:1.6;"><strong>Your ticket code covers ${count === 1 ? "the ticket" : `all ${count} tickets`} in this reservation.</strong> You still need to choose your reserved ${count === 1 ? "seat" : "seats"} using the button below.</p>`
    : "";
  ...
  const text = [
    ...
    ...(input.scanToken?.trim()
      ? [`Your ticket code covers ${count === 1 ? "the ticket" : `all ${count} tickets`} in this reservation. You still need to choose your reserved ${count === 1 ? "seat" : "seats"} using the link below.`, ""]
      : []),
