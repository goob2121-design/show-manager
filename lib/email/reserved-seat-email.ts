...
export function buildReservedSeatEmail(input: ReservedSeatEmailInput, logoSrc: string | null = CMMS_EMAIL_LOGO_SRC): ReservedSeatEmailContent {
  ...
  const seatSelectionCodeReminder = input.scanToken?.trim()
    ? `<p style="margin:0 0 24px;padding:12px 14px;background:#fff7ed;border-left:4px solid #d89b2b;color:#334155;font-size:14px;line-height:1.6;"><strong>Your ticket code covers ${count === 1 ? "the ticket" : `all ${count} tickets`} in this reservation.</strong></p>`
    : "";
  ...
  const text = [
    ...
    "",
    `Thank you for reserving ${reservedSeatPurchaseText(count)} for ${RESERVED_SEAT_EMAIL_EVENT_NAME}. We're looking forward to seeing you on ${customerShowDate}!`,
    "",
    "Your payment has been received successfully. The next step is to choose your reserved seats.",
    "",
    ...
    ...(input.scanToken?.trim()
      ? [`Your ticket code covers ${count === 1 ? "the ticket" : `all ${count} tickets`} in this reservation.`, ""]
      : []),
