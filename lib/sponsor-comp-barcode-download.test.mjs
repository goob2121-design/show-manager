...
const rendererUrl = new URL("./sponsor-comp-barcode-render.ts", import.meta.url);

test("download uses the canonical Code 128 renderer and returns PNG", async () => {
  const [route, renderer] = await Promise.all([readFile(routeUrl, "utf8"), readFile(rendererUrl, "utf8")]);
  assert.match(route, /renderSponsorCompBarcodePng\(tokenRow\.token\)/);
  assert.match(renderer, /generateCode128\(token\)/);
  assert.match(renderer, /sharp\(Buffer\.from\(barcode\.svg\)\)/);
