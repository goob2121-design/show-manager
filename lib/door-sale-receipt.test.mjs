...
test("persisted card door-sale snapshots retain their recorded payment method", () => {
  assert.deepEqual(getDoorSaleReceiptSnapshot({ door_payment_method: "card", door_unit_price: 10, door_sale_total: 20 }), {
    paymentMethod: "card",
    unitPrice: 10,
    total: 20,
  });
...
test("new ordinary Door Mode sales persist receipt snapshots in the canonical sale insert", async () => {
  ...
  assert.match(saleHandler, /door_payment_method: paymentMethod/);
...
test("Print Receipt actions use the returned ticket UUID and do not create another sale", async () => {
  ...
  assert.match(source, /print\/door-receipt\/\$\{encodeURIComponent\(latestDoorSaleReceiptId\)\}\?autoClose=1/);
  assert.match(source, /activity\.receiptTicketId/);
  assert.match(source, /print\/door-receipt\/\$\{encodeURIComponent\(activity\.receiptTicketId \?\? ""\)\}\?autoClose=1/);
});

test("receipt print page uses a fixed 58mm by 210mm thermal layout while remaining protected and read-only", async () => {
  ...
  assert.match(source, /@page \{ size: 58mm 210mm; margin: 0; \}/);
  assert.match(source, /door-sale-receipt-sheet \{ box-sizing: border-box; width: 58mm; break-inside: avoid; page-break-inside: avoid; \}/);
  assert.match(source, /door-sale-receipt \{ box-sizing: border-box; width: 58mm; padding: 3mm 5mm 2mm 5mm; break-inside: avoid; page-break-inside: avoid; overflow-wrap: anywhere; \}/);
  assert.match(source, /door-sale-receipt > \*, \.door-sale-receipt section > div, \.door-sale-receipt section > div > div \{ box-sizing: border-box; width: 100%; min-width: 0; \}/);
  assert.doesNotMatch(source, /58mm auto|58mm 110mm|3276mm|8\.5in 3\.5in|rotate\(|transform-origin|transform\s*:|scale\s*:|zoom\s*:|height: 8\.5in|height: 3\.5in/);
  assert.match(source, /print:min-h-0/);
  assert.match(source, /door-sale-receipt, \.door-sale-receipt \* \{ color: #000 !important; opacity: 1 !important; border-color: #000 !important; \}/);
  assert.match(source, /door-sale-receipt img \{ filter: grayscale\(1\) contrast\(2\); \}/);
  assert.match(source, /src="\/cmms-logo\.png"/);
  assert.match(source, /getDoorSaleReceiptSnapshot\(ticket\)/);
  assert.match(source, /ticket\.ticket_count === 1 \? "" : "s"/);
  assert.match(source, /formatCurrency\(receiptSnapshot\.unitPrice\)/);
  assert.match(source, /formatCurrency\(receiptSnapshot\.total\)/);
  assert.match(source, /receiptSnapshot\.paymentMethod === "cash" \? "Cash" : "Card"/);
...
test("receipt auto-print runs once after mount while manual print remains available", async () => {
  ...
  assert.match(receiptSource, /<AutoPrintOnMount closeAfterPrint=\{query\.autoClose === "1"\} \/>/);
  ...
  assert.match(autoPrintSource, /window\.addEventListener\("afterprint", handleAfterPrint\)/);
  assert.match(autoPrintSource, /window\.removeEventListener\("afterprint", handleAfterPrint\)/);
  assert.match(autoPrintSource, /if \(!closeAfterPrint \|\| hasRequestedCloseRef\.current\) return;[\s\S]*window\.close\(\)/);
