...
test("failure details appear only for failed deliveries with a stored reason", async () => {
  const source = await readFile(uiUrl, "utf8");
  assert.match(source, /delivery\.send_status === "failed" && delivery\.error_message/);
  assert.match(source, /Failure details|View failure/);
...
test("history API is authenticated, private, newest-first, and limited to 50", async () => {
  ...
  const selected = source.match(/\.select\("id,recipient,send_status,resend_message_id,error_message,sent_at,failed_at,created_at,[^"]+"\)/)?.[0] ?? "";
  assert.doesNotMatch(selected, /ticket_url_snapshot|provider_idempotency_key|subscriber_id|show_id/);
});
