...
const welcomeEmailUrl = new URL("./mailing-list-welcome-email.ts", import.meta.url);
...
test("welcome email keeps the branded renderer and includes calm Gmail guidance", async () => {
  const source = await readFile(welcomeEmailUrl, "utf8");
  assert.match(source, /renderEmailCenterEmail\(/);
  assert.match(source, /If you use Gmail, our emails may appear in Promotions\. If you don't see them in Primary, check Promotions, Spam, or Social\./);
  assert.match(source, /mailing-list-welcome-\$\{input\.subscriberId\}/);
});
