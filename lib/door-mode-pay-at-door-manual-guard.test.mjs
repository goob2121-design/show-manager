...
test("manual Pay at Door confirmation modal shows the selected authoritative admission before payment", async () => {
  ...
  assert.match(modal, /closePayAtDoorConfirmation/);
  assert.match(source, /function closePayAtDoorConfirmation\(\)[\s\S]*setPendingPayAtDoorTicket\(null\)[\s\S]*focusScanInput\(\)/);
});