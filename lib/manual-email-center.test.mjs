...
test("Email Center templates provide editable starter content and blank Custom fields", () => {
  ...
  assert.match(presale?.message ?? "", /As a CMMS Mailing List subscriber, you have Early Access to tickets/);
  assert.match(presale?.message ?? "", /public ticket sales begin/);
  assert.match(presale?.message ?? "", /Click the button below to access Early Access tickets\./);
