...
test("delivery uses the established upcoming show and its existing ticket link", async () => {
  const source = await readFile(deliveryUrl, "utf8");
  assert.match(source, /select\("id,name,show_date,ticket_link,ticket_sale_status,presale_starts_at,public_sale_starts_at,presale_access_code"\)/);
...
test("presale email reuses the mailing-list sender and branded renderer with conversational copy", async () => {
  ...
  assert.match(source, /Your Presale Access Code:/);
  assert.match(source, /const greeting = firstName \? `Hey \$\{firstName\}!`/);
  assert.match(source, /"Hey CMMS Family!"/);
  ...
  assert.match(delivery, /presaleCode: currentShow\.presale_access_code/);
});
