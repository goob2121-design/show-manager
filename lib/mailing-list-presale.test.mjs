...
test("active presale requires presale status and the configured time window", async () => {
  const source = await readFile(deliveryUrl, "utf8");
  assert.match(source, /show\.ticket_sale_status !== "presale"/);
  assert.match(source, /nowTime < presaleStart/);
  assert.match(source, /nowTime >= publicStart/);
  assert.match(source, /presaleStart !== null/);
  assert.match(source, /publicStart !== null/);
});
...
test("presale email reuses the mailing-list sender and includes the existing ticket CTA and inbox guidance", async () => {
  ...
  assert.match(source, /ctaLabel: "GET EARLY ACCESS TICKETS"/);
  assert.match(source, /ctaUrl: input\.ticketUrl/);
  assert.match(source, /Spam, Promotions, or Social folders/);
  assert.match(source, /Public ticket sales begin/);
});
