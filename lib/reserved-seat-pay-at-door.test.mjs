...
test("only regular paid reserved creation exposes Pay at Door intent", () => {
  ...
  assert.match(reservedAdmin, /formatReservedSeatPayAtDoorAmount/);
...
test("projection transfers transitional intent to exactly one canonical admission", () => {
  ...
  const quantityMigration = readFileSync("supabase/migrations/20260910_fix_reserved_seat_pay_at_door_quantity.sql", "utf8");
  assert.match(quantityMigration, /pay_at_door_amount = coalesce\(ticket\.pay_at_door_amount, ticket\.ticket_count \* 10\.00\)/);
...
test("cash opens the existing drawer only after success; card remains external", () => {
  ...
  assert.match(door, /openPayAtDoorConfirmation\(scannedTicket\)/);
