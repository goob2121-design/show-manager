...
test("presale validation blocks missing or unsafe show delivery data", () => {
  assert.deepEqual(validatePresaleEmailFields({}), [
    ...
    "The selected show does not have a ticket link.",
  ]);
  assert.deepEqual(validatePresaleEmailFields({ show_name: "CMMS", show_date: "October 3, 2026", presale_start: "September 1", public_sale_start: "September 8", ticket_link: "http://unsafe.example" }), ["The selected show's ticket link must use HTTPS."]);
});
...
test("presale show fields are server-authoritative and status mismatch is visible", async () => {
  ...
  assert.match(component, /This show is not currently set to Presale \/ Early Access\./);
