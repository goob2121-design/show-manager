...
test("presale merge fields render selected show data and friendly Eastern dates", () => {
  for (const field of ["presale_start", "public_sale_start", "presale_code"]) assert.ok(EMAIL_CENTER_MERGE_FIELDS.includes(field));
  ...
  assert.match(message.rendered, /As a CMMS Mailing List subscriber, you have Early Access to tickets/);
  assert.match(message.rendered, /public ticket sales begin September 8 at 8:00 PM/);
  assert.match(message.rendered, /Click the button below to access Early Access tickets\./);
  assert.match(message.rendered, /Your Presale Access Code:\nCMMSOCT3/);
  assert.equal(cta.rendered, "https://tickets.example.com/october");
  assert.equal(template?.ctaLabel, "EARLY ACCESS TICKETS");
  assert.doesNotMatch(message.rendered, /Gmail users|Primary inbox|Promotions|Spam or Social/);
...
test("presale validation blocks missing or unsafe show delivery data", () => {
  assert.deepEqual(validatePresaleEmailFields({}), [
    ...
    "This show does not have a presale access code in Show Details.",
    "This show does not have a valid ticket link in Show Details.",
  ]);
  assert.deepEqual(validatePresaleEmailFields({ show_name: "CMMS", show_date: "October 3, 2026", presale_start: "September 1", public_sale_start: "September 8", presale_code: "CMMSOCT3", ticket_link: "http://unsafe.example" }), ["This show does not have a valid ticket link in Show Details."]);
});
