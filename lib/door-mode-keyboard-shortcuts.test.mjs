...
test("Door Mode listener opens the canonical paid-door confirmation modal without creating a sale", async () => {
  ...
  assert.match(listener, /event\.preventDefault\(\);[\s\S]*keypadShortcutInFlightRef\.current = true;[\s\S]*openDoorSaleConfirmation\(quantity\)/);
  ...
  assert.match(source, /function openDoorSaleConfirmation\(quantity: number\)/);
  ...
  assert.match(source, /label: `Paid door \+\$\{quantity\} · \$\{paymentMethod === "cash" \? "Cash" : "Card"\}`/);
  assert.match(source, /undo:[\s\S]*\.delete\(\)[\s\S]*\.eq\("id", insertedTicket\.id\)/);
});

test("walk-up calculations remain unchanged and Pay at Door is counted separately", async () => {
  const source = await readFile(doorModeUrl, "utf8");
  const listener = source.slice(
    source.indexOf("function handleDoorModeKeypadShortcut"),
    source.indexOf("async function handleSubtractDoorSale"),
  ...
  assert.match(source, /const totalPaidAttendance = doorPaidTickets \+ prepaidOnlineTickets \+ reservedPayAtDoorAttendance;/);
  assert.match(source, /const totalRevenue = doorPaidRevenue \+ prepaidOnlineRevenue \+ reservedPayAtDoorRevenue;/);
