...
export function CashDrawerControls() {
  ...
  async function connect() { setBusy(true); setMessage(null); try { const selected = await requestCashDrawerPort(); setPort(selected); setCashDrawerPort(selected); setMessage("Cash drawer selected. Use Test / Open Drawer to test it."); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to connect the cash drawer."); } finally { setBusy(false); } }
  async function test() { if (!port) return; setBusy(true); setMessage(null); try { await openCashDrawerManually(); setMessage("Cash drawer opened."); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to open the cash drawer."); } finally { setBusy(false); } }
