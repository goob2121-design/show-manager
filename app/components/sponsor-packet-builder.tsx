    ...
    <main className="min-h-screen bg-stone-100 px-4 py-6 text-stone-900 sm:px-6">
      <style jsx global>{`
        ...
        .packet-section-heading { color: #0e7490; }
          ...
          .packet-admission-pass-header { padding-bottom: 0.1in !important; }
          .packet-admission-pass-logo { max-height: 0.62in !important; }
          .packet-admission-pass-brand { margin-top: 0.06in !important; font-size: 7.5pt !important; }
          .packet-admission-pass-title { margin-top: 0.04in !important; font-size: 18pt !important; line-height: 1.1 !important; }
          .packet-admission-pass-show { margin-top: 0.12in !important; gap: 0.06in 0.3in !important; padding: 0.1in 0.14in !important; font-size: 9pt !important; line-height: 1.2 !important; }
          .packet-admission-pass-holder { margin-top: 0.12in !important; }
          .packet-admission-pass-holder h3 { margin-top: 0.04in !important; font-size: 18pt !important; line-height: 1.12 !important; }
          .packet-admission-pass-holder p { margin-top: 0.04in !important; line-height: 1.15 !important; }
          .packet-admission-pass-seats { margin-top: 0.12in !important; padding: 0.1in 0.2in !important; }
          .packet-admission-pass-seats p { margin-top: 0.04in !important; line-height: 1.1 !important; }
          .packet-admission-pass-entry { margin-top: 0.12in !important; }
          .packet-admission-pass-entry > p { margin-top: 0.05in !important; line-height: 1.15 !important; }
          .packet-admission-pass-entry .ticket-code-block { padding: 0 !important; border: 0 !important; background: transparent !important; }
          .packet-admission-pass-entry .ticket-code-grid { margin-top: 0.06in !important; gap: 0.06in !important; }
          .packet-admission-pass-entry .ticket-code-surface { padding: 0.05in !important; }
          .packet-admission-pass-entry img[alt="Reservation barcode"] { max-width: 5.8in !important; max-height: 0.8in !important; }
          .packet-admission-pass-entry img[alt="Reservation QR code"] { max-width: 1.45in !important; max-height: 1.45in !important; }
          .packet-admission-pass-entry .ticket-code-meta { margin-top: 0.06in !important; gap: 0.02in !important; padding: 0.07in 0.1in !important; font-size: 8.5pt !important; line-height: 1.12 !important; }
          .packet-admission-pass-entry .ticket-code-meta p { font-size: 8.5pt !important; line-height: 1.12 !important; }
          .packet-admission-pass-page .packet-footer { margin-top: auto !important; padding-top: 0.06in !important; font-size: 7.5pt !important; line-height: 1.2 !important; }
          .packet-rsvp-page .packet-rsvp-content { margin-top: 0.2in !important; }
          .packet-rsvp-page .packet-rsvp-intro { margin-top: 0.1in !important; gap: 0.05in !important; }
          .packet-rsvp-page .packet-rsvp-action { margin-top: 0.18in !important; padding: 0.16in 0.24in !important; }
          .packet-rsvp-page .packet-rsvp-code { margin-top: 0.04in !important; }
          .packet-rsvp-page .packet-rsvp-website { margin-top: 0.1in !important; overflow-wrap: anywhere; }
          .packet-rsvp-page .packet-rsvp-qr { margin-top: 0.1in !important; width: 1.55in !important; height: 1.55in !important; }
          .packet-rsvp-page .packet-rsvp-instructions { margin-top: 0.08in !important; }
          .packet-rsvp-page .packet-rsvp-reminder { margin-top: 0.12in !important; padding-top: 0.1in !important; }
      ...
      <div className="packet-layout mx-auto grid max-w-[1500px] gap-6 xl:grid-cols-[minmax(24rem,0.8fr)_minmax(0,1.2fr)]">
        ...
        <section className="packet-preview-shell grid gap-5 rounded-2xl border border-stone-300 bg-stone-200 p-3 shadow-inner" aria-label="Sponsor packet live preview">
          ...
          {presentationSections.sponsorRsvp && sponsorCode ? <article className="packet-page packet-rsvp-page mx-auto min-h-[11in] w-full max-w-[8.5in] bg-white p-[0.7in] text-[11pt] leading-7 text-stone-900 shadow-lg">
            <header className="packet-rsvp-header border-b-2 border-emerald-800 pb-4 text-center"><Image src="/cmms-logo.png" alt="Cumberland Mountain Music Show" width={300} height={180} className="packet-rsvp-logo mx-auto h-24 w-auto object-contain" /><h2 className="packet-heading mt-3 text-3xl font-semibold">Sponsor RSVP &amp; Reserved Seating</h2></header>
          ...
          {admissionPass && selectedShow ? <article className="packet-page packet-admission-pass-page mx-auto min-h-[11in] w-full max-w-[8.5in] bg-white p-[0.7in] text-stone-900 shadow-lg"><header className="packet-admission-pass-header border-b-2 border-stone-900 pb-5 text-center"><Image src="/cmms-logo.png" alt="Cumberland Mountain Music Show" width={320} height={120} className="packet-admission-pass-logo mx-auto max-h-24 w-auto object-contain" /><p className="packet-admission-pass-brand mt-4 text-xs font-black uppercase tracking-[0.3em] text-amber-700">Cumberland Mountain Music Show</p><h2 className="packet-admission-pass-title mt-2 text-3xl font-black tracking-[0.08em]">OFFICIAL ADMISSION PASS</h2></header><section className="packet-admission-pass-show mt-5 grid grid-cols-2 gap-x-8 gap-y-3 rounded-xl border border-stone-300 bg-stone-50 p-4 text-sm"><div><strong className="block text-[10px] uppercase tracking-widest text-stone-500">Show</strong>{selectedShow.name}</div><div><strong className="block text-[10px] uppercase tracking-widest text-stone-500">Date</strong>{formatSponsorPacketDate(selectedShow.show_date ?? "")}</div><div><strong className="block text-[10px] uppercase tracking-widest text-stone-500">Venue</strong>{selectedShow.venue || "Venue TBD"}</div><div><strong className="block text-[10px] uppercase tracking-widest text-stone-500">Doors / Show</strong>{selectedShow.guest_arrival_time || "6:00 PM"} / {selectedShow.show_start_time || "TBD"}</div></section><section className="packet-admission-pass-holder mt-5 text-center"><p className="text-xs font-black uppercase tracking-[0.22em] text-amber-700">{admissionPass.admissionLabel}</p><h3 className="mt-2 text-3xl font-black">{admissionPass.sponsorName}</h3>{admissionPass.contactName && admissionPass.contactName !== admissionPass.sponsorName ? <p className="mt-1 text-base text-stone-600">Issued to {admissionPass.contactName}</p> : null}<p className="mt-2 text-sm font-bold">{admissionPass.quantity} admission{admissionPass.quantity === 1 ? "" : "s"}</p></section>{(() => { const seating = sponsorAdmissionSeatSummary(admissionPass); return <section className="packet-admission-pass-seats mt-5 rounded-2xl border-2 border-stone-900 px-6 py-5 text-center"><p className="text-xs font-black uppercase tracking-[0.25em] text-stone-600">{seating.heading}</p>{seating.lines.map((line, index) => <p key={line} className={index === seating.lines.length - 1 ? "mt-1 text-3xl font-black" : "mt-2 text-xl font-black"}>{line}</p>)}</section>; })()}<section className="packet-admission-pass-entry mt-5 text-center"><p className="text-base font-black uppercase tracking-[0.16em]">Present this page at the door</p><div className="mt-2"><ReservationTicketCode scanToken={admissionPass.scanToken} format={selectedShow.ticket_code_format} purchaserName={admissionPass.contactName || admissionPass.sponsorName} ticketCount={admissionPass.quantity} seatLabels={admissionPass.seats.map((seat) => seat.seat_id)} printable /></div><p className="mt-2 font-mono text-sm font-bold tracking-[0.08em]">Entry Code: {admissionPass.scanToken}</p>{admissionPass.quantity > 1 ? <p className="mt-2 text-sm font-semibold">One scan admits all {admissionPass.quantity} admissions listed on this pass.</p> : null}</section><PacketFooter page={pageNumberFor("admission-pass")} total={totalPages} /></article> : null}
