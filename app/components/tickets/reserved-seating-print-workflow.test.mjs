...
test("selected chair cards omit admission codes and emphasize assignment details", async () => {
  ...
  assert.match(selectedCards, /Reserved Seating[\s\S]*print:text-black/);
  assert.match(selectedCards, /print:text-\[32px\][^>]*print:font-black[^>]*print:text-black/);
  assert.match(selectedCards, /print:text-\[19px\][^>]*print:font-bold[^>]*print:text-black/);
  assert.match(selectedCards, /max-w-full whitespace-nowrap[\s\S]*\{card\.seatId\}/);
  assert.match(selectedCards, /print:whitespace-nowrap[\s\S]*print:text-black[\s\S]*\{printName\}/);
  assert.match(selectedCards, /seat-card selected-reserved-seat-card/);
  assert.match(source, /html\.dark \.selected-reserved-seat-card,[\s\S]*html\.dark \.selected-reserved-seat-card \*[\s\S]*color: #000000 !important;/);
  assert.match(source, /html\.dark \.selected-reserved-seat-card \{ background: #ffffff !important; \}/);
  assert.match(selectedCards, /\{card\.seatId\}[\s\S]*\{card\.seatExplanation\}/);
  assert.match(source, /\.selected-reserved-seat-card img\[src="\/cmms-logo\.png"\]/);
  assert.match(source, /filter: saturate\(1\.65\) contrast\(1\.08\) !important;/);
  assert.match(selectedCards, /print:max-h-\[48px\] print:max-w-\[140px\]/);
  assert.match(source, /-webkit-print-color-adjust: exact !important;/);
