...
import { processDueScheduledEmailCampaigns } from "@/lib/scheduled-email-campaign";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [presaleResults, generalResults] = await Promise.all([
      processDueScheduledPresaleCampaigns({ supabase, origin: request.nextUrl.origin, apiKey: process.env.RESEND_API_KEY }),
      processDueScheduledEmailCampaigns({ supabase, origin: request.nextUrl.origin, apiKey: process.env.RESEND_API_KEY }),
    ]);
    return NextResponse.json({ success: true, processed: presaleResults.length + generalResults.length, presaleResults, generalResults });
