...
function logSupabaseError(operation: string, error: { code?: string; message?: string; details?: string; hint?: string }) {
  console.error(`[Sponsor RSVP] ${operation} failed`, {
...
async function lookup(code: string) {
  ...
  const { data: sponsor, error } = await supabase.from("sponsor_library")
    ...
    .maybeSingle();
  if (error) {
    logSupabaseError("Sponsor lookup", error);
    throw error;
  }
  console.info("[Sponsor RSVP] Sponsor lookup completed", { found: Boolean(sponsor) });
  ...
  const { data: show, error: showError } = await supabase.from("shows")
    ...
    .maybeSingle();
  if (showError) {
    logSupabaseError("Upcoming show lookup", showError);
    throw showError;
  }
  console.info("[Sponsor RSVP] Upcoming show lookup completed", { found: Boolean(show) });
  if (!show) return { sponsor, show: null, rsvp: null };

  const { data: rsvp, error: rsvpError } = await supabase.from("sponsor_show_rsvps")
    .select("status,guest_count,note,responded_at")
    .eq("sponsor_id", sponsor.id)
    .eq("show_id", show.id)
    .maybeSingle();
  if (rsvpError) {
    logSupabaseError("Existing RSVP lookup", rsvpError);
    throw rsvpError;
  }
  console.info("[Sponsor RSVP] Existing RSVP lookup completed", { found: Boolean(rsvp) });
  return { sponsor, show, rsvp };
}

export async function POST(request: NextRequest) {
  console.info("[Sponsor RSVP] Route entered");
  try {
    ...
    console.info("[Sponsor RSVP] Request received", { action, sponsorCode: code });
    ...
    if (error) {
      logSupabaseError("RSVP upsert", error);
      throw error;
    }
    return NextResponse.json({ success: true, status, guestCount });
  } catch (error) {
    console.error("[Sponsor RSVP] Unhandled request failure", error);
