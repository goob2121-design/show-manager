...
export default async function SquareIntegrationStatusPage({ params }: PageProps) {
  ...
  const [{ data: show }, { data: events }] = await Promise.all([
    ...
    supabase
      ...
      .select("id, event_id, event_type, payment_id, order_id, line_item_uid, catalog_variation_id, show_id, show_name, result, ticket_count, email_present, seat_link_created, email_sent, error_message, received_at, imported_at")
          ...
          <div className="mt-4 overflow-x-auto rounded-xl border border-stone-200">
            <table className="min-w-full divide-y divide-stone-200 text-sm">
              <thead className="bg-stone-50 text-left text-xs font-bold uppercase tracking-[0.12em] text-stone-500">
                <tr><th className="px-3 py-2">Received</th><th className="px-3 py-2">Result</th><th className="px-3 py-2">Order</th><th className="px-3 py-2">Show</th><th className="px-3 py-2">Qty</th><th className="px-3 py-2">Email?</th><th className="px-3 py-2">Seat Link?</th><th className="px-3 py-2">Email Sent</th></tr>
              </thead>
              <tbody className="divide-y divide-stone-100">
                {typedEvents.length > 0 ? typedEvents.map((event) => (
                  <tr key={event.id}>
                    ...
                    <td className="px-3 py-2">{event.email_present ? "Yes" : "No"}</td>
                    <td className="px-3 py-2">{event.seat_link_created ? "Yes" : "No"}</td>
                    <td className="px-3 py-2">{event.email_sent ? "Yes" : "No"}</td>
                  </tr>
                )) : (
                  <tr><td className="px-3 py-5 text-stone-500" colSpan={8}>No Square webhook events recorded yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
