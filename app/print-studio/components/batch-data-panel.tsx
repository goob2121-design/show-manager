...
function numberInputValue(value: number | undefined | null) {
  return value ?? "";
}
  ...
  const renderSequentialTicketNumbering = () => (
    <CollapsibleSection title="Sequential Ticket Numbering" defaultOpen badge={`${firstTicket}-${lastTicket}`}>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <label className={labelClass}>Starting ticket number<input className={inputClass} type="number" value={numberInputValue(settings.startingNumber)} onChange={(event) => onChange({ startingNumber: integerValue(event.target.value, settings.startingNumber ?? 1) })} /></label>
        <label className={labelClass}>Quantity<input className={inputClass} type="number" min="1" max="1000" value={numberInputValue(settings.quantity)} onChange={(event) => onChange({ quantity: positiveInteger(event.target.value, settings.quantity ?? 1) })} /></label>
        <label className={labelClass}>Increment<input className={inputClass} type="number" min="1" value={numberInputValue(settings.increment)} onChange={(event) => onChange({ increment: positiveInteger(event.target.value, settings.increment ?? 1) })} /></label>
        <label className={labelClass}>Number padding<input className={inputClass} type="number" min="0" max="12" value={numberInputValue(settings.padding)} onChange={(event) => onChange({ padding: Math.max(0, integerValue(event.target.value, settings.padding ?? 0)) })} /></label>
        ...
        {settings.mode === "sequential" ? (
          <div className="grid gap-4">
            ...
            <CollapsibleSection title="Sequential Seat Sequence" description="Optional generated seats override the default Seat value." defaultOpen={false} badge={settings.seatSequenceEnabled ? `${seatStart}-${seatEnd}` : "Off"}>
              <label className="mb-3 flex items-center gap-2 text-sm font-bold text-slate-200"><input type="checkbox" checked={Boolean(settings.seatSequenceEnabled)} onChange={(event) => onChange({ seatSequenceEnabled: event.target.checked })} />Generate seat sequence</label>
              {settings.seatSequenceEnabled ? (
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <label className={labelClass}>Starting seat number<input className={inputClass} type="number" value={numberInputValue(settings.seatStart)} onChange={(event) => onChange({ seatStart: integerValue(event.target.value, settings.seatStart ?? 1) })} /></label>
                  <label className={labelClass}>Seat increment<input className={inputClass} type="number" min="1" value={numberInputValue(settings.seatIncrement)} onChange={(event) => onChange({ seatIncrement: positiveInteger(event.target.value, settings.seatIncrement ?? 1) })} /></label>
                  <label className={labelClass}>Seat number padding<input className={inputClass} type="number" min="0" max="12" value={numberInputValue(settings.seatPadding)} onChange={(event) => onChange({ seatPadding: Math.max(0, integerValue(event.target.value, settings.seatPadding ?? 0)) })} /></label>
                </div>
              ) : null}
            </CollapsibleSection>
          </div>
